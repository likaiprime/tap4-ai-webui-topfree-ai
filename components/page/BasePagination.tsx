...
function LinkTag({
  ...
  return (
    <Link
      ...
      title={title}
      className={cn(
        'flex-center size-8 rounded-[4px] border border-transparent bg-transparent text-white/40 hover:cursor-pointer hover:bg-[#FFFFFF]',
        className,
      )}
    >
      {children}
  ...
  if (type === 'page') {
    return (
      <LinkTag
        href={route}
        title={page.toString()}
        className={page === currentPage ? 'border-white bg-[#FFFFFF] text-white' : ''}
      >
        {page}
      </LinkTag>
    );
